...
store* curr_store = NULL;
int g_fd = -1;
int cache_valid = 0;
size_t cache_idx = 0;
char* cache_data[NUM_OF_PODS + 1] = {NULL};
char* cache_key = NULL;
...
int init_info(store* ptr) {
    ...
    for(idx = 0; idx < NUM_OF_PODS; idx++){
        ...
        assert(sem_init(&pod->protect, 1, 1) == 0);
        memset(pod->distinct_keys, 0, sizeof(pod->distinct_keys));
        memset(pod->vals, 0, sizeof(pod->vals));
...
int kv_store_create(char *name){
    ...
    curr_store = mmap(NULL, STORE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);

    if(curr_store == MAP_FAILED){
        close(fd);
        perror("Mmap error");
        return -1;
    }
    init_info(curr_store);
    if(munmap(curr_store, STORE_SIZE) < 0){
        close(fd);
        perror("Munmap error");
        return -1;
    }
